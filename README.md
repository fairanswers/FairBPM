...
## Overview
Fair BPM is a workflow library. With it, you can describe a process have the computer do that process.  It's like a programming language that non-programmers can use. Business users can change how it works without waiting for a developer.
...
If a process is a recipe for a sandwich, a job is a copy of the recipe PLUS what happened when you ran each activity.  If someone ordered 2 sandwiches at the same time, we'd want to make sure we did all the steps for each sandwich.  Jobs keep up with which activities have run, and which ones still need to run.
